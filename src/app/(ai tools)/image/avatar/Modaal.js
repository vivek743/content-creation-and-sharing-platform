...
export default function Modaal() {
  ...
  const uploadImage = async (file) => {
    try {
      ...
      // Create FormData and append the necessary fields for Cloudinary
      const formData = new FormData();
      formData.append("file", file); // Attach the file
      formData.append("upload_preset", "intellect"); // Your Cloudinary upload preset
      formData.append("api_key", process.env.NEXT_PUBLIC_CLOUDINARY_API_KEY); // Cloudinary API key
      formData.append("public_id", `images/ImageAvatar/${Date.now()}`); // Optional: Assign a public ID
      ...
      if (data?.secure_url) {
        setUploadedIMG(data.secure_url); // Set the uploaded image URL
      } else {
        throw new Error("Image upload failed");
      }
    } catch (error) {
      errorToast(error.message); // Handle error appropriately
    }
  };
