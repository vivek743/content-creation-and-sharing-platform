...
function page() {
  ...
  useEffect(() => {
    ...
    const createVideo = async () => {
      try {
        const response = await fetch("/api/videos/image-to-video", {
          ...
          signal: controller.signal, // Attach the abort signal to cancel if the component unmounts
        });
    ...
    return () => {
      controller.abort(); // Clean up any pending request
    };
  }, [uploadedIMG]);
  ...
  const handleFileDrop = (event) => {
    ...
    const size = file.size / 1024 / 1024; // File size in MB
    const fileType = file.type.split("/")[0];

    if (size > 10) {
      fileSizeError("File size should be less than 10MB");
    } else if (fileType !== "image") {
      fileSizeError("Only image files are allowed");
    } else {
  ...
  const uploadImage = async (file) => {
    try {
      ...
      formData.append("public_id", `videos/Img2Motion/${Date.now()}`);
